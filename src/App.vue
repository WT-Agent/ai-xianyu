  ...
  <div class="app-container">
    ...
    <main ref="inputCardRef" class="glass-card input-group">
      ...
      <div class="options-row" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
        <div class="selector-group">
          <label class="selector-label">闲置品类</label>
          <div class="style-selector">
            <button 
        ...
        <div class="selector-group">
          <label class="selector-label">转手诉求</label>
          <div class="style-selector">
            <button 
      ...
      <div class="selector-group">
        <div style="display: flex; justify-content: space-between; align-items: center;">
          ...
          <div style="display: flex; gap: 0.5rem;">
            ...
            <button class="text-link-btn" @click="showGuideModal = true">咸鱼闲置高价快速变现与防坑指南</button>
          </div>
        </div>
        <textarea 
          v-model="userInput" 
...
const appTitle = ref(appConfig.title || '网腾无限AI - 咸鱼/转转二手闲置爆款专家');
...
const metricsList = [
  {
    "key": "secondHandTrustworthiness",
    "label": "闲置真实保真信任度"
  },
  {
    "key": "searchKeywordsExposure",
    "label": "搜索关键词曝光力"
  },
  {
    "key": "itemConditionClarity",
    "label": "成色缺陷告知清晰度"
  },
  {
    "key": "inquiryTransactionIncentive",
    "label": "私信咨询成交促成力"
  },
  {
    "key": "ruleConstraintFrictionless",
    "label": "交易规则无摩擦分"
  }
];
const guideRules = [
  {
    "title": "坦诚告知缺陷更出单",
    "advice": "在文案和照片中主动展示细微划痕或瑕疵，建立绝对真实信任感，避免到货扯皮",
    "avoid": "切忌隐瞒瑕疵导致买家收到货后申请退款拒签"
  },
  {
    "title": "转手原因要接地气",
    "advice": "给出合理的转手原因（如“公司发的新电脑”、“搬家带不走”），比“帮朋友卖”更具说服力",
    "avoid": "避免让人觉得是职业贩子批量倒买倒卖"
  },
  {
    "title": "提前约定交易规则",
    "advice": "在文案结尾写明“售出非假不退不换”、“到货请当面签收”、“不接受大刀砍价”，减少无用沟通",
    "avoid": "严禁接受微信线下私下转账等无平台担保交易"
  }
];
...
const displayResultText = computed(() => {
  ...
  return result.value.replace(/\[XIANYU_SCORES\][\s\S]*?\[\/XIANYU_SCORES\]/g, '').trim();
});

const parseAiScores = (rawText: string) => {
  const match = rawText.match(/\[XIANYU_SCORES\](.*?)\[\/XIANYU_SCORES\]/);
...
const loadHistory = () => {
  try {
    const raw = localStorage.getItem('xianyu_history_records');
...
const saveHistory = () => {
  localStorage.setItem('xianyu_history_records', JSON.stringify(historyList.value));
};
